...
interface YearlyMetricsViewProps {
  data: ProcessedTeacherData[];
}

const YearlyMetricsView: React.FC<YearlyMetricsViewProps> = ({ data }) => {
  const [selectedMetric, setSelectedMetric] = useState('visits');

  // Group data by teacher and year
  const yearlyData = useMemo(() => {
    ...
    const grouped = data.reduce((acc, item) => {
      ...
      // Extract year from period (assuming format like "2024-01" or "January 2024")
      const year = item.period?.includes('-') 
        ? item.period.split('-')[0] 
        : item.period?.split(' ').pop() || 'Unknown';
      ...
      acc[teacher][year].visits += item.totalVisits || 0;
      acc[teacher][year].cancellations += item.cancellations || 0;
      acc[teacher][year].lateCancellations += item.lateCancellations || 0;
      acc[teacher][year].noShows += item.noShows || 0;
      acc[teacher][year].newMembers += item.newClients || 0;
      acc[teacher][year].retained += item.retainedClients || 0;
      acc[teacher][year].converted += item.convertedClients || 0;
      acc[teacher][year].classes += item.totalClasses || 0;
      acc[teacher][year].uniqueMembers += item.uniqueClients || 0;
      acc[teacher][year].revenue += item.totalRevenue || 0;
      acc[teacher][year].retentionRate = item.retentionRate || 0;
      acc[teacher][year].conversionRate = item.conversionRate || 0;
  ...
  const allYears = useMemo(() => {
    ...
    if (yearlyData && typeof yearlyData === 'object') {
      Object.values(yearlyData).forEach(teacherData => {
        if (teacherData && typeof teacherData === 'object') {
          Object.keys(teacherData).forEach(year => {
  ...
  const teachers = useMemo(() => {
    ...
    return Object.keys(yearlyData).filter(teacher => teacher && teacher.trim() !== '');
  }, [yearlyData]);

  // Calculate totals for each year and metric
  const calculateTotals = (metric: string) => {
    if (!allYears || !teachers || !yearlyData) {
      return [];
    }
    return allYears.map(year => {
      const total = teachers.reduce((sum, teacher) => {
        const teacherData = yearlyData[teacher];
        const yearData = teacherData && teacherData[year];
        return sum + (yearData && yearData[metric] ? yearData[metric] : 0);
      }, 0);
      return {
        year,
        total
  ...
  const calculateYearOverYearGrowth = (metric: string) => {
  ...
  const growthData = calculateYearOverYearGrowth(selectedMetric);

  // Render yearly metric table
          ...
          <CardTitle className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold flex items-center gap-2 text-white">
                {selectedMetricConfig.label} - Year over Year
                <CalendarDays className="h-5 w-5 text-blue-400 animate-pulse" />
              </div>
              <div className="text-sm text-white/80 font-medium">Yearly performance analysis with growth trends</div>
            </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {allYears.map(year => (
                    <TableHead key={year} className="text-white font-bold text-center min-w-[140px] px-4">
              ...
              <TableBody>
                {teachers.map((teacher, index) => {
                  const teacherTotal = allYears.reduce((sum, year) => {
                    const teacherData = yearlyData[teacher];
                    const yearData = teacherData && teacherData[year];
                    return sum + (yearData && yearData[selectedMetric] ? yearData[selectedMetric] : 0);
                  }, 0);
                  ...
                  return (
                    ...
                    >
                      ...
                      {allYears.map(year => {
                        const teacherData = yearlyData[teacher];
                        const yearData = teacherData && teacherData[year];
                        const value = yearData && yearData[selectedMetric] ? yearData[selectedMetric] : 0;
                        return (
                          <TableCell key={year} className="text-center min-w-[140px] font-semibold text-slate-800 px-4">
                ...
                <TableRow className="border-t-2 border-slate-300/50 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900">
                  <TableCell className="font-bold sticky left-0 z-20 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-r border-white/20 min-w-[180px] text-white px-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold">Yearly Total</span>
                    </div>
                  </TableCell>
                  {growthData.map(({ year, total, growth }) => (
                    <TableCell key={year} className="text-center font-bold text-white min-w-[140px] px-4">
        ...
        <Card className="animate-fade-in bg-white/90 backdrop-blur-xl border border-white/40 shadow-luxury rounded-2xl">
          <CardContent className="p-12 text-center">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-xl font-semibold text-slate-800">No yearly data available</p>
              <p className="text-slate-600 max-w-md">
                No data available for yearly metrics view. Please ensure you have uploaded data spanning multiple years.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-blue-500/10 via-blue-600/5 to-transparent border border-blue-200/50 shadow-lg hover:shadow-xl transition-all duration-300 rounded-2xl overflow-hidden">
          <CardHeader className="pb-3 bg-gradient-to-r from-blue-100/80 to-blue-50/80 border-b border-blue-200/50">
            <CardTitle className="text-sm flex items-center gap-2 text-blue-700">
              ...
              Total Visits (All Years)
            </CardTitle>
          </CardHeader>
          <CardContent className="p-4">
            <div className="text-3xl font-bold text-blue-800">{totalMetrics.totalVisits.toLocaleString()}</div>
            <p className="text-xs text-blue-600 mt-1 font-medium">Across all teachers & years</p>
          </CardContent>
        </Card>
...
export default YearlyMetricsView;
