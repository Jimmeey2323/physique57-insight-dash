...
const PerformanceMetricCard: React.FC<PerformanceMetricCardProps> = ({
  ...
  return <Card className={`card-hover bg-white/60 backdrop-blur-sm transition-all duration-300 ${isAnimated ? 'hover:shadow-md hover:-translate-y-1' : ''} ${getStatusBgColor()}`} onClick={onCustomClick}>
      <CardContent className="pt-6 bg-white border-gray-700 border-2 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="w-full">
            <div className="flex items-center gap-1 text-lg font-bold tracking-wide text-gray-900">
              {title}
            ...
            <div className={`text-2xl font-bold mt-1 ${getStatusColor()} ${isAnimated ? 'group-hover:scale-105 transition-transform' : ''}`}>
              {displayValue}
              {secondaryValue && <span className={`ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${status === 'positive' ? 'bg-green-100 text-green-800' : status === 'neutral' ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'}`}>
                  {secondaryValue.includes('%') && <Percent className="mr-1 h-3 w-3" />}
                  {secondaryValue}
                </span>}
            </div>
            {trend && trend.value !== undefined && <div className={`text-xs mt-1 flex items-center ${trend.value >= 0 ? 'text-green-500' : 'text-red-500'}`}>
